...
public class Lists02 {
    ...
    public static void main(String[] args) {
        ...
        b.removeAll(c);//remove all elements of "c" from "b" so theese removed elements are not available in "b"
        ...
        h.set(1, 9.99);//set() method is for updating an element by using index. We changed index:1
        ...
        for (int i = 0; i < k.size(); i++) {    // we use size() method instead of length() in "list"
            k.set(i, k.get(i) * 1.1);   // get() method gives you an element . for example  get(3) gives us the element in index:3
        }           //  idx(0)==> 5000*1,1==600 // set() method will change the elements one by one with for-loop
        System.out.println(k);
